...
osmo.dataSvg = class {
  ...
  initSplash(_width) {
    ...
    // SCROLL TEXT
    //
    let textLoc = new this.PAPER.Point(
      raster.position.x - (raster.width * this.scale) / 3,
      raster.position.y + raster.height * this.scale * 0.65
    );
    ...
    let end = new this.PAPER.Point(
      start.x + (raster.width * this.scale) / 2 - textWidth,
      start.y
    );
    ...
    var triangle = new this.PAPER.Path.RegularPolygon(
      new this.PAPER.Point(end.x, end.y + size / 4),
      3,
      size
    );
    triangle.rotate(90);
  ...
  init(q) {
    ...
    //
    //
    this.backgroundTweenItem = new this.PAPER.Shape.Circle(
    ...
    if (this.quality == "High") {
      ...
      raster.on("load", function () {
        ...
        raster.position.x =
          (osmo.scroll.paperWidth * 3) / 4 + (raster.width * self.scale) / 2;
        //
