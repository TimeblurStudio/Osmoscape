	...
	init(){
		...
		$.getJSON(dataURL, function( data ) {
			...
			let dataWaitInterval = setInterval(function(){
				if(osmo.datasvg){
					if(osmo.datasvg.mainScroll != null && !jQuery.isEmptyObject(self.mergedMasks) && !jQuery.isEmptyObject(self.mergedLegends)){
		...
		//
		let masksURL = 'assets/data/mergedMasks.json' + '?v=' + window.version;
		console.log('mergedMasksURL: ' + masksURL);
		$.getJSON(masksURL, function( data ) {
			self.mergedMasks = data;
   ...
   */
	loadDataset(id, early=true){
		if (this.datasets.hasOwnProperty(id)) {
			...
			//
			let maskdata = this.mergedMasks[id];
      ...
      */
			...
			//
			//
			let maskpromise = this.maskLoad(title, maskdata, id, morder);
			let legendpromise = this.legendLoad(title, legenddata, id);
			//
			if(early){
