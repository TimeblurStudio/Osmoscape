...
osmo.Scroll = class {
  ...
  init() {
    ...
    paper.view.onMouseMove = function (event) {
      if (!osmo.navinteract.isOnDiv) osmo.legendinteract.mouseMoved(event);
    };

    paper.view.onMouseDown = function (event) {
      if (!osmo.navinteract.isOnDiv) osmo.popupinteract.mouseClicked(event);
    };

    paper.view.onMouseUp = function (event) {
      if (!osmo.navinteract.isOnDiv) osmo.popupinteract.mouseUp(event);
    };
    //
    //
  }
  ...
  initModal(start_opened) {
    ...
    //
    if (start_opened) self.toggleModal();
  }
    ...
    $(document).ready(function () {
      ...
      $(".speaker").hover(
        ...
        function () {
          ...
          //		$('.vlCtrl').hide();
          setTimeout(function () {
            if (!isOnCtrl || isOnSpeaker) $(".vlCtrl").fadeOut();
          }, 1500);
        }
      );
      //
      ...
      var easing = {
        inCubic: function (t, b, c, d) {
          ...
          return (
            b +
            c *
              (1.7 * tc * ts - 2.05 * ts * ts + 1.5 * tc - 0.2 * ts + 0.05 * t)
          );
        },
        ...
        customSin: function (t, b, c, d) {
          ...
          return (
            b +
            c * (81 * tc * ts + -210 * ts * ts + 190 * tc + -70 * ts + 10 * t)
          );
        },
      };
        ...
        animation: function () {
          if (this.off) {
            ...
            if (this.offCurStep >= this.offMaxStep - step1) {
              let progress =
                (step1 + this.offCurStep - this.offMaxStep) / step1;
              let progressB = fromTo(1, backLen, 1 - progress);
              volObj.speakF.setAttribute(
                "stroke-dasharray",
                len * progress + "," + len * 1.05
              );
              volObj.speakF.setAttribute(
                "stroke-dashoffset",
                (-len * (1 - progress)) / 2 + ""
              );
              volObj.speakB.setAttribute(
                "stroke-dasharray",
                len * progressB + "," + len * 1.05
              );
              volObj.speakB.setAttribute(
                "stroke-dashoffset",
                (-len * (1 - progressB)) / 2 + ""
            ...
            [volObj.arcBigF, volObj.arcBigB].forEach((el) => {
              ...
              el.setAttribute(
                ...
                transforms.translate(
                  this.curPosBig.x + dxBig * amplitudeB,
                  y,
                  ""
                ) + transforms.scale(scale, scale)
              );
            ...
            [volObj.arcSmF, volObj.arcSmB].forEach((el) => {
              ...
              el.setAttribute(
                "transform",
                transforms.translate(
                  this.curPosSm.x + dxSm * amplitudeS,
                  y,
                  ""
                ) + transforms.scale(scale, scale)
              );
      ...
      var drag = {
        ...
        animateDrag: function () {
          ...
          [volObj.arcBigF, volObj.arcBigB].forEach((el) => {
            play.curPosBig.x =
              -this.pointBig.x * (scaleFactor - 1) + scaleDxBig;
          ...
          if (progress > 0.5) {
            ...
            volObj.arcBigF.setAttribute(
              "stroke-dasharray",
              bgLen * prgForBig + "," + bgLen * 1.05
            );
            volObj.arcBigF.setAttribute(
              "stroke-dashoffset",
              (-bgLen * (1 - prgForBig)) / 2 + ""
          ...
          if (progress <= 0.5 && progress > 0) {
            ...
            volObj.arcSmF.setAttribute("visibility", "visible");
            volObj.arcSmF.setAttribute(
              "stroke-dasharray",
              smLen * prgForSm + "," + smLen * 1.05
            );
            volObj.arcSmF.setAttribute(
              "stroke-dashoffset",
              (-smLen * (1 - prgForSm)) / 2 + ""
  ...
  loadHQ() {
    ...
    //
    let please_wait_spinner =
      '<div id="percentage" style="color: #b97941; font-weight: 400;"></div><br><div class="sk-three-bounce"><div class="sk-child sk-bounce1" style="background-color: #b97941"></div><div class="sk-child sk-bounce2" style="background-color: #b97941"></div><div class="sk-child sk-bounce3" style="background-color: #b97941"></div></div>';
    ...
    //
    // APPROACH 2
    this.load_fetch_retry(
      "assets/images/SCROLL_cs6_ver23_APP_final_300ppi-HIGH-or8.png",
      3,
      "High",
      "HQscroll"
    );
  ...
  loadRQ() {
    ...
    // APPROACH 2
    this.load_fetch_retry(
      "assets/images/SCROLL_cs6_ver23_APP_final_600ppi-RETINA-or8.png",
      3,
      "Retina",
      "RQscroll"
    );
  ...
  loadMQ() {
    ...
    // APPROACH 2
    this.load_fetch_retry(
      "assets/images/SCROLL_cs6_ver23_APP_final_150ppi-LOW-or8.png",
      3,
      "Mobile",
      "MQscroll"
    );
  ...
  load_fetch_retry(url, n, type, typeID) {
    ...
    return fetch(url)
      ...
      .catch(function (error) {
        if (n === 1) {
          $(".pg-loaded").html(
            '<p style="color: #b97941">Failed to load main scroll. Please try later!</p>'
          );
          throw error;
        }
