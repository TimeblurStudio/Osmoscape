...
osmo.panAndZoom = class {
  ...
  init() {
    ...
    $("#main-scroll-canvas").on("wheel", function (event) {
      //
      if (!osmo.scroll.loaded.svgdata || !osmo.scroll.loaded.HQimage) return;
      //
      ...
      if (osmo.scroll.hitPopupMode != "focused") {
        osmo.legendinteract.hitMaskEffect(
          new osmo.scroll.PAPER.Point(0, 0),
          "scrolling"
        );
      ...
      //
      let fac =
        1.005 / (osmo.scroll.PAPER.view.zoom * osmo.scroll.PAPER.view.zoom);
      //
      ...
      if (osmo.scroll.hitPopupMode != "focused") {
        ...
        //
        osmo.scroll.PAPER.view.center = osmo.pzinteract.changeCenter(
          osmo.scroll.PAPER.view.center,
          deltaValX,
          0,
          fac
        );
    ...
    if (osmo.legendsvg.maskLayer.visible == false) {
      ...
      Object.keys(osmo.legendsvg.popupBBoxes).forEach(function (key) {
        ...
        for (let i = 0; i < allpaths.length; i++)
          if (
            allpaths[i].bounds.rightCenter.x > xMin &&
            allpaths[i].bounds.leftCenter.x < xMax
          )
            enabled = true;
        if (enabled) {
          console.log("Enabled: " + osmo.legendsvg.popupBBoxes[key]["mask"].id);
          osmo.legendsvg.popupBBoxes[key]["mask"].visible = true;
        } else osmo.legendsvg.popupBBoxes[key]["mask"].visible = false;
        //
      });
      //
      ...
      cevent.point.x += xMin;
      if (!osmo.navinteract.isOnDiv) osmo.legendinteract.mouseMoved(cevent);
      //
    }
  }
  ...
  changeCenter(oldCenter, deltaX, deltaY, factor, restricted = true) {
    ...
    if (restricted) {
      ...
      //
      if (
        oldCenter.y * this.PAPER.view.zoom - osmo.scroll.paperHeight / 2 <= 0 &&
        deltaY > 0
      )
        oldCenter.y = osmo.scroll.paperHeight / (2 * this.PAPER.view.zoom);
      if (
        oldCenter.y * this.PAPER.view.zoom >
          -osmo.scroll.paperHeight / 2 +
            osmo.scroll.paperHeight * this.PAPER.view.zoom &&
        deltaY < 0
      )
        oldCenter.y =
          -osmo.scroll.paperHeight / (2 * this.PAPER.view.zoom) +
          osmo.scroll.paperHeight;
      //
    }
    return oldCenter;
  }
