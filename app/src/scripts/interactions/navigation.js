...
osmo.navigationInteraction = class {
  ...
  loadNav() {
    ...
    this.PAPER.project.importSVG(navPath, function (item) {
      ...
      item.position.x =
        (osmo.scroll.paperWidth * 3) / 4 + osmo.datasvg.scrollWidth / 2;
      item.opacity = 0.01;
      //
  ...
  initNav() {
    ...
    $(".jump").click(function (el) {
      ...
      let locX = osmo.scroll.PAPER.project.getItem({ name: "nav-ch" + chap_id })
        .bounds.left;
      let w = osmo.scroll.PAPER.project.getItem({ name: "nav-ch" + chap_id })
        .bounds.width;
      //
      console.log(chap_id);
      console.log(locX);
      console.log(w);
      //
      if (w > osmo.scroll.paperWidth) locX += osmo.scroll.paperWidth / 2;
      else locX += w / 2;
      //
      let dur = 2000;
      ...
      osmo.navinteract.navTweenItem.tween(
        ...
        {
          position: new osmo.scroll.PAPER.Point(
            locX,
            osmo.scroll.PAPER.view.center.y
          ),
        },
        {
  ...
  hitNavEffect() {
    //
    var hitResult = this.navLayer.hitTest(
        ...
        if (this.currentNavLoc != navLoc) {
          ...
          for (let i = 0; i < elements.length; i++) {
            ...
            if (ele.hasClass("selected")) {
              ...
              ele[0].firstChild.src = ele[0].firstChild.src.replace(
                "_selected",
                "_default"
              );
            }
